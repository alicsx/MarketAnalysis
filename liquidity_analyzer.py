...
def analyze_final_levels(data, context):
    ...
    for idx in high_indices:
        ...
        if context.get("DXY") == "Bullish":
            score += 30; thesis += "DXY Strength confirms. "
        if context.get("SPX") == "Bearish":
            score += 10; thesis += "Risk-Off sentiment. "
        if peak['RSI_14'] > 68:
            score += 15; thesis += "Overbought RSI. "
        if peak['Close'] < peak['ISA_9']:
            score += 15; thesis += "Rejected by Ichimoku. "
        levels.append({"type": "high", "price": peak['High'], "validity_score": score, "trade_thesis": thesis})
    ...
    for idx in low_indices:
        ...
        if context.get("DXY") == "Bearish":
            score += 30; thesis += "DXY Weakness confirms. "
        if context.get("SPX") == "Bullish":
            score += 10; thesis += "Risk-On sentiment. "
        if peak['RSI_14'] < 32:
            score += 15; thesis += "Oversold RSI. "
        if peak['Close'] > peak['ISA_9']:
            score += 15; thesis += "Supported by Ichimoku. "
        levels.append({"type": "low", "price": peak['Low'], "validity_score": score, "trade_thesis": thesis})
...
def main():
    ...
    try:
        ...
        primary_data = yf.download(PRIMARY_SYMBOL, start=start_date, end=end_date, progress=False)
